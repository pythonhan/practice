"""
    ...
    日期：09/20/2017
    版本:V11.0
    功能：网络爬虫,用beautifulsoup4解决数据解析问题,获取所有城市的AQI,将获取的CSV指数保存成csv文件,使用pandas进行数据处理，数据可视化
    requests模块
...
def main():
    ...
    # top50
    top50_cities = clean_aqi_data.sort_values(by= ['AQI']).head(50)   #升序排列
    top50_cities.plot(kind='bar', x='City', y='AQI',title='空气质量最好的50个城市',
                      figsize=(20, 10))    #bar表示柱状图
